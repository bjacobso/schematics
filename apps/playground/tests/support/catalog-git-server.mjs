...
const workspaceDir = resolve(repoRoot, "tmp/catalog-git-workspace");
const manifestPath = resolve(repoRoot, "tmp/catalog-git-workspace.json");
...
run(
  "node",
  [
    resolve(repoRoot, "examples/catalog/dist/deploy-cli-bin.js"),
    "pull",
    "--dir",
    workspaceDir,
    "--account",
...
const server = spawn(
  "node",
  [
    resolve(repoRoot, "examples/catalog/dist/cli.js"),
    "web",
    "--dir",
    workspaceDir,
    "--port",
...
await new Promise((resolveProcess, reject) => {
  server.once("exit", (code, signal) => {
    ...
    else reject(new Error(`catalog-config server exited with code ${code ?? signal}`));
  });
});
