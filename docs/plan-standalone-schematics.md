...
- Keep development end-to-end inside `packages/schematics`: local packages, local examples, local playground, local HTTP server.
...
```
packages/schematics/playground
        │
        ▼
...
Do not change host app routes, runtime handlers, ontology authoring logic, or host-specific proxy code while this gate is active. Any temporary host compatibility remains outside the extractable tree at `packages/schematics-compat`.
...
│ packages/schematics/turbo    │        │ turbo.standalone.json        │
└──────────────────────────────┘        └──────────────────────────────┘
```
...
pnpm --dir packages/schematics serve:smoke
```
...
```
packages/schematics/
├── core/         @schematics/core
├── protocol/     @schematics/protocol
├── agent/        @schematics/agent
├── react/        @schematics/ide
├── server/       @schematics/server
├── examples/     @schematics/examples
├── playground/   package-local Vite app
├── README.md
└── package.json  private workspace command runner
```

The temporary compatibility package is intentionally outside this tree at `packages/schematics-compat`.
...
- The standalone server boots with `pnpm --dir packages/schematics/server dev`.
- The standalone server exposes a built `schematics-server` binary through `@schematics/server`.
- The combined local command boots server plus playground with `pnpm --dir packages/schematics dev`.
- The built standalone server can serve the built playground and `/v1` API from one Node process with `pnpm --dir packages/schematics serve`.
...
- The temporary compatibility package was moved to `packages/schematics-compat`.
...
- A copied-out `packages/schematics` tree installs, formats, tests, typechecks, builds, builds the playground, and serves `/` plus `/v1` without the host repository.
...
- **Deployed playground URL**: this is a post-extraction publication step, not an in-repo package implementation requirement. The extractable tree includes `.github/workflows/playground-pages.yml`, but that nested workflow is inert inside the Open Ontology monorepo. `gh api repos/bjacobso/open-ontology/pages` returns 404, so there is no active Pages site to cite. The PR preview URL serves the host Open Ontology app, not the isolated package playground, so it should not be listed as the Schematics playground URL. Add the concrete URL to `packages/schematics/README.md` only after the tree is copied to a standalone repository with GitHub Pages enabled, or after an explicit package-only deploy target is approved for this monorepo.
...
- [x] No host-scoped package imports under `packages/schematics`.
- [x] No host-scoped package names under `packages/schematics`.
...
- [x] `@schematics/server` boots standalone with `pnpm --dir packages/schematics/server dev`.
- [x] `packages/schematics/playground` runs against the standalone server with `pnpm --dir packages/schematics dev`.
- [x] `pnpm --dir packages/schematics serve` serves the built playground and standalone `/v1` API together.
...
pnpm --dir packages/schematics playground:build
pnpm --dir packages/schematics install --frozen-lockfile
pnpm --dir packages/schematics test
pnpm --dir packages/schematics typecheck
pnpm --dir packages/schematics build
pnpm --dir packages/schematics serve:smoke
